...
| `dynesty.pool.Pool` | `SerialMapBackend` / `ThreadedMapBackend` / `DistributedMapBackend` / `NestedSampler(...; parallel, map_backend, queue_size)` | A | replacement | `test/test_parallel.jl`, `test/test_static_sampler.jl`, `test/test_dynamic_sampler.jl` | not needed | Ordered Julia-native map replacement with queue controls and sampler-level configuration; covers initial live-point evaluation and proposal/evolve queue mapping. Python Pool shape and `use_pool` dictionary are intentionally replaced; bound updates remain serial. |
