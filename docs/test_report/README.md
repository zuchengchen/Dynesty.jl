...
- `generate_assets.py`: 从已有 benchmark 输出审计并生成报告表格，同时复制 overlay PNG。
- `tables/`: 报告使用的 CSV、LaTeX 表格和 benchmark metadata。
- `figures/`: 从 `examples/output/parallel_cost_compare/plots/` 复制来的 9 张 `*julia_vs_python.png`。
...
该命令只读取 `examples/output/parallel_cost_compare/`，并写入：
