...
| `dynesty.sampler.Sampler` | `NestedSampler` internals | A | implemented | `test/test_static_sampler.jl` | statistical/invariant | Static sampler engine including run loop, proposal/evolve queue, deterministic task RNGs, final live points, result conversion, bound updates, blobs, and checkpoint restore. |
