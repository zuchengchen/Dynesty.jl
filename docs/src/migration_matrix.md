...
| `dynesty.pool.Pool` | `SerialMapBackend` / `ThreadedMapBackend` / `DistributedMapBackend` / `PoolUsage` / `NestedSampler(...; parallel, map_backend, queue_size, pool_usage, use_pool)` | A | replacement | `test/test_parallel.jl`, `test/test_static_sampler.jl`, `test/test_dynamic_sampler.jl` | not needed | Ordered Julia-native map replacement with queue controls and sampler-level configuration; covers initial live-point evaluation, proposal/evolve queue mapping, and opt-in built-in bound bootstrap mapping for `update_bound`/`PoolUsage(bounds=true)`. `pool_usage` is the recommended Julia policy API, while `use_pool` dictionaries/named tuples parse Python keys into `PoolUsage`. Python Pool worker objects are intentionally not copied or serialized. |
...
| `dynesty.sampler.Sampler` | `NestedSampler` internals | A | implemented | `test/test_static_sampler.jl` | statistical/invariant | Static sampler engine including run loop, proposal/evolve queue, deterministic task RNGs, final live points, result conversion, bound updates, opt-in bound bootstrap backend use, blobs, and checkpoint restore. |
...
| `dynesty.utils._parse_pool_queue` | `_normalize_queue_size` / `_get_map_backend` / `PoolUsage` parser | B | replacement | `test/test_parallel.jl`, `test/test_static_sampler.jl` | not needed | Julia backend, queue, `pool_usage`, and compatibility `use_pool` parsing for low-level maps and sampler constructor keywords; explicit `map_backend` plus `queue_size` raises `ArgumentError`, and simultaneous `pool_usage` plus `use_pool` raises `ArgumentError`. |
